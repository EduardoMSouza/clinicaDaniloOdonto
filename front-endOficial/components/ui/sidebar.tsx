...
function getSidebarStateFromStorage(): boolean {
    ...
    return true // Estado padrão (expandido)
}
...
function SidebarProvider({
    ...
    const setOpen = React.useCallback(
        (value: boolean | ((value: boolean) => boolean)) => {
            ...
            // Salva no localStorage (mais confiável que cookies)
            try {
                localStorage.setItem(SIDEBAR_STORAGE_KEY, JSON.stringify(openState))
            } catch (error) {
                console.warn('Erro ao salvar estado do sidebar no localStorage:', error)
            }

            // Também salva no cookie para compatibilidade
            try {
                document.cookie = `sidebar_state=${openState}; path=/; max-age=${60 * 60 * 24 * 7}; SameSite=Lax`
            } catch (error) {
                console.warn('Erro ao salvar estado do sidebar no cookie:', error)
            }
        },
        [setOpenProp, open]
    )
